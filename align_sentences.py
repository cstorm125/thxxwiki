...
if __name__ == "__main__":
    ...
    for en_path, th_path in tqdm.tqdm(zip(en_paths,th_paths)):
        ...
        for i in tqdm(range(len(sent_en2)//bs+1)):
            for j in tqdm(range(len(sent_th2)//bs+1)):
                matched_sentences_th, scores = match_sentences(sent_en2[i*args.bs:(i+1)*args.bs],\
                                                               sent_th2[j*args.bs:(j+1)*args.bs], _model)
                res_en_th = pd.DataFrame({'en_text':sent_en2[i*bs:(i+1)*bs],'th_text':matched_sentences_th,'use_score':scores,'id_en':id_en})
